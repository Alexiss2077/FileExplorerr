...
namespace FileExplorerr
{
    ...
    internal class ConexionDialog : Form
    {
        ...
        private void BuildUI()
        {
            ...
            Text = _tipo switch { SqlViewerForm.DbTipo.Postgres => "Conectar a PostgreSQL", SqlViewerForm.DbTipo.Maria => "Conectar a MariaDB", SqlViewerForm.DbTipo.SqlServer => "Conectar a SQL Server", _ => "Conectar" };
            ...
            {
                SqlViewerForm.DbTipo.Postgres => (Color.FromArgb(95, 189, 235), "🐘  Conexión PostgreSQL"),
                SqlViewerForm.DbTipo.Maria => (Color.FromArgb(60, 180, 120), "🌿  Conexión MariaDB"),
                SqlViewerForm.DbTipo.SqlServer => (Color.FromArgb(230, 140, 60), "🗄  Conexión SQL Server"),
                _ => (Color.FromArgb(72, 202, 188), "Conexión")
            };

            var header = new Panel { Height = 44, Dock = DockStyle.Top, BackColor = Color.FromArgb(26, 26, 32) };
            header.Controls.Add(new Label { Text = headerText, Dock = DockStyle.Fill, Font = new Font("Segoe UI", 11F, FontStyle.Bold), ForeColor = accentColor, TextAlign = ContentAlignment.MiddleLeft, Padding = new Padding(14, 0, 0, 0) });
            Controls.Add(header);
            ...
            AddRow("Puerto:", ref txtPuerto, esPostgres ? "5432" : esSqlServer ? "1433" : "3306");
            AddRow("Base de datos:", ref txtBd, "");
            AddRow("Usuario:", ref txtUser, esPostgres ? "postgres" : esSqlServer ? "sa" : "root");
            AddRow("Contraseña:", ref txtPass, "", pass: true);

            if (esSqlServer)
            {
                chkTrusted = new CheckBox { Text = "Autenticación de Windows (Trusted Connection)", Location = new Point(14, y), Size = new Size(374, 24), ForeColor = Color.FromArgb(110, 140, 180), BackColor = Color.Transparent, Font = new Font("Segoe UI", 9F) };
                chkTrusted.CheckedChanged += (s, e) => { txtUser.Enabled = !chkTrusted.Checked; txtPass.Enabled = !chkTrusted.Checked; };
        ...
        private void BtnOk_Click(object? sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(bd)) { MessageBox.Show("Especifica el nombre de la base de datos.", "Campo requerido", MessageBoxButtons.OK, MessageBoxIcon.Warning); DialogResult = DialogResult.None; return; }

            if (_tipo == SqlViewerForm.DbTipo.Postgres)
                ConnectionString = $"Host={host};Port={puerto};Database={bd};Username={user};Password={pass};Timeout=10;CommandTimeout=60";
            else if (_tipo == SqlViewerForm.DbTipo.Maria)
                ConnectionString = $"Server={host};Port={puerto};Database={bd};User={user};Password={pass};ConnectionTimeout=10;DefaultCommandTimeout=60";
            else // SqlServer
            {
                bool trusted = chkTrusted?.Checked == true;
                if (trusted)
                    ConnectionString = $"Server={host},{puerto};Database={bd};Integrated Security=True;TrustServerCertificate=True;Connect Timeout=10;";
                else
                    ConnectionString = $"Server={host},{puerto};Database={bd};User Id={user};Password={pass};TrustServerCertificate=True;Connect Timeout=10;";
            }
        }
    }
    ...
    internal class NombreTablaDialog : Form
    {
        ...
        public NombreTablaDialog(string sugerido)
        {
            ...
            btnOk.FlatAppearance.BorderColor = Color.FromArgb(35, 134, 54);
            btnOk.Click += (s, e) => { NombreTabla = txtNombre.Text.Trim(); if (string.IsNullOrWhiteSpace(NombreTabla)) { MessageBox.Show("Ingresa un nombre.", "Requerido", MessageBoxButtons.OK, MessageBoxIcon.Warning); DialogResult = DialogResult.None; } };
