...
namespace FileExplorerr
{
    ...
    public class CoverSearcher
    {
        ...
        /// <summary>
        /// Busca cover consultando todas las fuentes en paralelo.
        /// </summary>
        public async Task<CoverSearchResult?> BuscarMejorCover(string artista, string titulo)
        {
            ...
            var tasks = new[]
            {
                BuscarEnITunes(artistaLimpio, tituloLimpio, artista, titulo),
                BuscarEnLastFm(artistaLimpio, tituloLimpio, artista, titulo),
                BuscarEnSpotify(artistaLimpio, tituloLimpio, artista, titulo)
            };
            ...
            string artistaOriginal, string tituloOriginal)
        {
            try
            {
                ...
                for (int i = 0; i < Math.Min(results.GetArrayLength(), 20); i++)
                {
                    ...
                    string coverUrl = (item.GetProperty("artworkUrl100").GetString() ?? "").Replace("100x100", "600x600");
                    double similitud = CalcularSimilitudAvanzada(artistaLimpio, tituloLimpio, artistaAPI, tituloAPI);
                    if (similitud > 0.4)
                        covers.Add(new CoverSearchResult { Url = coverUrl, Fuente = "iTunes", Similitud = similitud, Artista = artistaAPI, Titulo = tituloAPI });
                }
                ...
                foreach (var track in tracksArray)
                {
                    try
                    {
                        ...
                        if (track.TryGetProperty("image", out var imageArray))
                        {
                            ...
                            if (largeImage.ValueKind != JsonValueKind.Undefined && largeImage.TryGetProperty("#text", out var textProp))
                            {
                                string coverUrl = textProp.GetString() ?? "";
                                if (!string.IsNullOrWhiteSpace(coverUrl) && !coverUrl.EndsWith("back"))
                                {
                                    double similitud = CalcularSimilitudAvanzada(artistaLimpio, tituloLimpio, artistaLfm, tituloLfm);
                                    if (similitud > 0.4)
                                        covers.Add(new CoverSearchResult { Url = coverUrl, Fuente = "Last.fm", Similitud = similitud, Artista = artistaLfm, Titulo = tituloLfm });
                                }
                            }
                        }
                    }
                    ...
                    try
                    {
                        ...
                        if (track.TryGetProperty("album", out var album) && album.TryGetProperty("images", out var images))
                        {
                            ...
                            if (imageArray.TryGetProperty("url", out var urlProp))
                            {
                                ...
                                double similitud = CalcularSimilitudAvanzada(artistaLimpio, tituloLimpio, artistaSpotify, tituloSpotify);
                                if (similitud > 0.4)
                                    covers.Add(new CoverSearchResult { Url = coverUrl, Fuente = "Spotify", Similitud = similitud, Artista = artistaSpotify, Titulo = tituloSpotify });
                            }
                        }
                    }
                    catch { }
            ...
            string artistaEncontrado, string tituloEncontrado)
        {
            if (string.IsNullOrEmpty(artistaBuscado) || string.IsNullOrEmpty(tituloBuscado) ||
                ...
                return 0;
            if (artistaBuscado == artistaEncontrado && tituloBuscado == tituloEncontrado) return 1.0;
            ...
            return Math.Min(1.0, similitudArtista * 0.35 + similitudTitulo * 0.50 + similitudPalabras * 0.15);
        }
        ...
        private double CalcularSimilitudFuzzy(string s1, string s2)
        {
            ...
            return Math.Pow(1.0 - ((double)distancia / maxLongitud), 0.8);
        }

        private double CalcularSimilitudPalabras(string s1, string s2)
        {
            var palabras1 = s1.Split(new[] { ' ', '-', '&' }, StringSplitOptions.RemoveEmptyEntries).Where(p => p.Length > 2).ToList();
            var palabras2 = s2.Split(new[] { ' ', '-', '&' }, StringSplitOptions.RemoveEmptyEntries).Where(p => p.Length > 2).ToList();
            if (palabras1.Count == 0 || palabras2.Count == 0) return 0;
        ...
        private string ExtraerArtistaPrincipal(string artista)
        {
            ...
            foreach (var sep in separadores)
            {
                ...
                if (index > 0) { resultado = resultado.Substring(0, index); break; }
            }
        ...
        public string NormalizarTexto(string texto)
        {
            ...
            string[] stopwords = { "feat.", "ft.", "featuring", "remix", "official", "video", "audio", "lyrics", "letra", "hq", "4k", "prod.", "producer", "album version", "version", "edit", "extended", "radio edit", "remaster", "remastered", "live", "acoustic", "unplugged", "cover", "instrumental", "explicit", "clean", "deluxe", "edition", "ep", "single" };
            foreach (var word in stopwords) { int index = texto.IndexOf(word); if (index != -1) texto = texto.Substring(0, index); }
